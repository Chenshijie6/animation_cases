<template>
    <div id="drawArea" style="position: fixed;">
        <canvas id="canvas" style="background-color: #000;cursor: pointer;"></canvas>
        <div class="operation"
            style="position: absolute; left: 20px; top: 22px; border: 1px solid white; color:dodgerblue; padding: 0px 5px;">
            分叉数目:<span id="statmsg">{{ statMsg }}</span>
            <!-- <span>(提示：点击页面不同区域可在相应位置绘制树)</span><br /> -->
            <label for="numSelect">选择初始树枝数：</label>
            <select id="numSelect" style="width: 100px;height:22px; background-color: dodgerblue; margin: 5px auto;"
                v-model="selectedNum">
                <option value="2">2</option>
                <option value="3">3</option>
                <option value="4">4</option>
            </select><br />
            <button id="btnClear" style="width: 100px; background-color: dodgerblue; margin: 5px auto;"
                @click="clearCanvas">清除页面</button>
            <button id="btnStop_Start" style="width: 100px; background-color: dodgerblue; margin: 5px auto;"
                @click="toggleStart">开始/暂停绘画</button>
            <button id="btnReload" style="width: 100px; background-color: dodgerblue; margin: 5px auto;"
                @click="reloadCanvas">重新绘制</button>
        </div>
    </div>
</template>
...
const stretch = 600 / canvasHeight;
const ySpeed = 3 / stretch;

const statMsg = ref(0);
const selectedNum = ref('3');
const tree = new Tree();

const drawStart = (x = centerX) => {
    for (let i = 0; i < Number(selectedNum.value); i++) {
        new Branch({
            p: new Vector(x, canvasHeight),
            v: new Vector(Util.random(-1, 1), -ySpeed),
            r: 15 / stretch,
...
const clearCanvas = () => {
    ...
    statMsg.value = 0;
    if (tree.timer) clearInterval(tree.timer);
};
...
onMounted(() => {
    ...
    //     const x = e.pageX - drawArea.offsetLeft;
    //     const y = e.pageY - drawArea.offsetTop;
    //     if (tree.timer) clearInterval(tree.timer);
    //     drawStart(x);
    // });
});
