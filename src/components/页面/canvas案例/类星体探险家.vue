...
const mouse = reactive({
    ...
    update: (x, y, pointerId) => {
        mouse.x = x * dpr
        mouse.y = (innerHeight - y) * dpr
        mouse.touches.add(pointerId)
    },
    remove: (pointerId) => mouse.touches.delete(pointerId)
})
let canvas, gl, program, buffer, vertices, dpr

dpr = Math.max(1, .5 * window.devicePixelRatio)
